const baluf007 = require("../../baluff-007-protocol/lib/index.js");
...
const connect = ({ ipAddress, port, onConnect, onError }) => {
        ...
        onConnect: () => {
            ...
            requestScanSlot(async () => {
                ...
                try {
                    ...
                    if (sc1 !== baluf007.statusCodes.OK) {
                        error = baluf007.errorFromStatusCode({ statusCode: sc1 });
                    }
    ...
    const maybeReconfigure = () => {
        return configQueue.add(async () => {
            ...
            if (sc1 !== baluf007.statusCodes.OK) {
                return {
                    error: Error(`failed to read io link master config`, { cause: baluf007.errorFromStatusCode({ statusCode: sc1 }) })
                };
            }
            ...
            if (sc2 === baluf007.statusCodes.ACCESS_DENIED) {
            ...
            } else if (sc2 !== baluf007.statusCodes.OK) {
                return {
                    error: Error(`failed to write io link master config`, { cause: baluf007.errorFromStatusCode({ statusCode: sc2 }) })
                };
            }
        ...
        const poll = ({ onRead, pollIntervalMs }) => {
            ...
            const next = () => {
                ...
                Promise.resolve().then(async () => {
                    ...
                    if (sc2 !== baluf007.statusCodes.OK) {
                        return {
                            error: baluf007.errorFromStatusCode({ statusCode: sc2 })
                        };
                    }
    ...
    const claimIoLink = ({
        ...
        const writeCyclicProcessData = async ({ offset, data }) => {
            ...
            if (sc1 !== baluf007.statusCodes.OK) {
                return {
                    error: Error("write io link process data failed", { cause: baluf007.errorFromStatusCode({ statusCode: sc1 }) })
                };
            }
    ...
    const detectOrGetCarriers = async ({ dataType, maxNumberCarriers = 999 }) => {
        ...
        if (statusCode === baluf007.statusCodes.NO_CARRIER_IN_RANGE) {
            return {
                detectedCarriers: []
            };
        } else if (statusCode !== baluf007.statusCodes.OK) {
            return {
                error: baluf007.errorFromStatusCode({ statusCode })
            }
        }
    ...
    const requestCumulatedModeAndFlush = async () => {
        ...
        if (sc1 !== baluf007.statusCodes.OK) {
            return {
                error: Error(`flushing list / going to live mode failed`, { cause: baluf007.errorFromStatusCode({ statusCode: sc1 }) })
            };
        }
        ...
        if (sc2 !== baluf007.statusCodes.OK) {
            return {
                error: Error(`going to cumulated mode failed`, { cause: baluf007.errorFromStatusCode({ statusCode: sc2 }) })
            };
        }
...
module.exports = {
    connect,
    statusCodes: baluf007.statusCodes,
    errorFromStatusCode: baluf007.errorFromStatusCode
};
